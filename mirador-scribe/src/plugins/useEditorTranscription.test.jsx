...
function mount(adapter, overrides = {}) {
  ...
  const options = {
    ...
    foregroundTranscriptionIsBlocked: () => false,
...
describe('foreground editor transcription', () => {
  it('does not start foreground work while durable transcription owns the page', async () => {
    const adapter = { transcribeAnnotation: vi.fn() };
    const { applyTransformResult, options, setStatusMessage } = mount(adapter, {
      foregroundTranscriptionIsBlocked: () => true,
    });
    ...
    expect(options.setOperationBusyState).not.toHaveBeenCalled();
