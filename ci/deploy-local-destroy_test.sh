...
grep -F "timeout-minutes: \${{ inputs.mode == 'destroy' && 180 || inputs.mode == 'apply' && inputs.environment_name == 'production' && 240 || 120 }}" \
  <<<"$deploy_job_header" >/dev/null || {
  echo "Terraform deploy job timeout does not isolate the extended destroy and production recovery windows" >&2
  exit 1
}
...
managed_destroy_timeout_seconds=$((180 * 60))
[ "$serverless_wait_seconds" -eq 7200 ]
[ $((managed_destroy_timeout_seconds - serverless_wait_seconds)) -ge 3600 ] || {
