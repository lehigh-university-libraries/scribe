...
if FAKE_HEAD_SHA="$later_main_sha" run_release_tag >"$TEST_DIR/later.out" 2>"$TEST_DIR/later.err"; then
  echo "release tag accepted a later main HEAD instead of the reviewed merge" >&2
  exit 1
fi
...
echo "Release tag creation is retry-idempotent and refuses a moved remote tag."
