...
docker exec -i "${SOURCE_DB}" mariadb --user=root --password="${DB_ROOT_PASSWORD}" "${DB_NAME}" <<'SQL'
...
docker exec -i "${RESTORE_DB}" mariadb --user=root --password="${DB_ROOT_PASSWORD}" "${DB_NAME}" < "${TEMP_DIR}/database.sql"
