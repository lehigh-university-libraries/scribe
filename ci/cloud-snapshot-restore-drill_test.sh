...
[
  ...
  {
    "name": "scribe-backups-daily-1",
    "status": "READY",
    "autoCreated": true,
    "sourceDisk": "https://compute.googleapis.com/compute/v1/projects/${project}/zones/${zone}/disks/scribe-mariadb-backups",
    "creationTimestamp": "${now}",
    "diskSizeGb": "150",
    "labels": {"managed_by": "terraform", "instance": "scribe"}
  }
...
if [[ "$group $kind $action" == "compute disks create" ]]; then
  if [[ "${MOCK_FAIL_BACKUPS_CREATE:-false}" == "true" && "$name" == *-backups ]]; then
...
if [[ "$group $kind $action" == "compute disks delete" ]]; then
  if [[ "${MOCK_CLEANUP_FAIL:-false}" == "true" && "$name" == *-backups ]]; then
    exit 1
  fi
...
export PRODUCTION_BACKUP_DISK=scribe-mariadb-backups
export PRODUCTION_BACKUP_DISK_SELF_LINK=https://www.googleapis.com/compute/v1/projects/scribe-test/zones/us-east5-b/disks/scribe-mariadb-backups
...
reset_fixture() {
  ...
  unset MOCK_OLD_SNAPSHOTS MOCK_WRONG_SOURCE MOCK_FAIL_BACKUPS_CREATE MOCK_MISSING_MARKER MOCK_CLEANUP_FAIL
}
...
grep -q -- 'device-name=restore-backups,mode=ro,boot=no,auto-delete=no' "$MOCK_GCLOUD_LOG"
...
reset_fixture
export MOCK_FAIL_BACKUPS_CREATE=true
if "$ROOT_DIR/ci/cloud-snapshot-restore-drill.sh" >/dev/null 2>&1; then
...
find "$MOCK_GCLOUD_STATE/disks" -type f -name '*-backups' -print -quit | grep -q .
