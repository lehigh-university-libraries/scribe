...
require_fixed 'working-directory: .rollback-source' .github/workflows/terraform-deploy.yaml
# shellcheck disable=SC2016 # Match literal workflow shell variables.
require_fixed 'merge-base --is-ancestor "$PREVIOUS_SHA" "$CURRENT_SHA"' .github/workflows/terraform-deploy.yaml
...
while IFS='|' read -r environment_name configuration_name terraform_variable; do
  ...
  require_fixed "export ${environment_name}=\"\$(jq" .github/workflows/terraform-deploy.yaml
