...
require_inputs() {
  ...
  : "${PRODUCTION_BACKUP_DISK:?PRODUCTION_BACKUP_DISK is required}"
  : "${PRODUCTION_BACKUP_DISK_SELF_LINK:?PRODUCTION_BACKUP_DISK_SELF_LINK is required}"
  ...
  [[ "$PRODUCTION_BACKUP_DISK" =~ ^[a-z]([-a-z0-9]{0,61}[a-z0-9])?$ ]] || fail "invalid production backup disk" || exit 2
  [[ "$PRODUCTION_DATA_DISK" != "$PRODUCTION_VOLUMES_DISK" &&
    "$PRODUCTION_DATA_DISK" != "$PRODUCTION_BACKUP_DISK" &&
    "$PRODUCTION_VOLUMES_DISK" != "$PRODUCTION_BACKUP_DISK" ]] || fail "production disks must be distinct" || exit 2
  ...
  [[ "$PRODUCTION_BACKUP_DISK_SELF_LINK" == */projects/"$GCLOUD_PROJECT"/zones/"$PRODUCTION_ZONE"/disks/"$PRODUCTION_BACKUP_DISK" ]] ||
    fail "production backup-disk self link is not exact" || exit 2
...
restore_backup_disk="${resource_prefix}-backups"
readonly resource_prefix drill_instance restore_data_disk restore_volumes_disk restore_backup_disk run_id run_attempt
...
delete_drill_disk() {
  local name="$1" expected_run_id="${2:-$run_id}" expected_run_attempt="${3:-$run_attempt}" json
  is_drill_name "$name" || return 1
  [[ "$name" != "$PRODUCTION_DATA_DISK" && "$name" != "$PRODUCTION_VOLUMES_DISK" && "$name" != "$PRODUCTION_BACKUP_DISK" ]] || return 1
...
cleanup_current() {
  ...
  delete_drill_disk "$restore_backup_disk" || status=1
  return "$status"
}
...
mount_clone /dev/disk/by-id/google-restore-backups /mnt/restore-backups

backup="$(find /mnt/restore-backups/mariadb/primary -maxdepth 1 -type f -name '*-primary.sql.gz' -printf '%T@\t%p\n' | sort -nr | awk -F '\t' 'NR == 1 {print $2}')"
[[ -n "$backup" && -f "$backup" && ! -L "$backup" && -s "$backup" ]]
backup_epoch="$(stat -c %Y -- "$backup")"
now_epoch="$(date -u +%s)"
((now_epoch >= backup_epoch && now_epoch - backup_epoch <= 172800))
gzip -t -- "$backup"
marker="/mnt/restore-backups/mariadb/.last-success"
[[ -f "$marker" && ! -L "$marker" ]]
marker_date="$(tr -d '\n' <"$marker")"
...
run_drill() {
  local data_source_json volumes_source_json backup_source_json data_snapshot volumes_snapshot backup_snapshot
  local data_epoch volumes_epoch backup_epoch oldest_epoch newest_epoch skew source_instance_json network subnetwork
  ...
  backup_source_json="$(validate_source_disk "$PRODUCTION_BACKUP_DISK" "$PRODUCTION_BACKUP_DISK_SELF_LINK")"
  ...
  backup_snapshot="$(latest_daily_snapshot "$PRODUCTION_BACKUP_DISK_SELF_LINK")" || fail "no exact READY daily backup-disk snapshot exists"
  data_epoch="$(snapshot_epoch "$data_snapshot")"
  volumes_epoch="$(snapshot_epoch "$volumes_snapshot")"
  backup_epoch="$(snapshot_epoch "$backup_snapshot")"
  oldest_epoch="$data_epoch"
  newest_epoch="$data_epoch"
  ((volumes_epoch < oldest_epoch)) && oldest_epoch="$volumes_epoch"
  ((backup_epoch < oldest_epoch)) && oldest_epoch="$backup_epoch"
  ((volumes_epoch > newest_epoch)) && newest_epoch="$volumes_epoch"
  ((backup_epoch > newest_epoch)) && newest_epoch="$backup_epoch"
  ...
  create_restore_disk "$restore_backup_disk" "$backup_snapshot" "$backup_source_json"
  ...
  gcloud compute instances create "$drill_instance" \
    ...
    --disk="name=${restore_backup_disk},device-name=restore-backups,mode=ro,boot=no,auto-delete=no" \
  ...
  for _ in $(seq 1 "$POLL_ATTEMPTS"); do
    ...
    if grep -q '^SCRIBE_RESTORE_PROBE_OK$' <<<"$serial"; then
      echo "Cloud snapshot restore drill passed for snapshots $(jq -r '.name' <<<"$data_snapshot"), $(jq -r '.name' <<<"$volumes_snapshot"), and $(jq -r '.name' <<<"$backup_snapshot")."
      return 0
    fi
