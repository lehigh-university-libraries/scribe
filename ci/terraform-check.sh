...
check_command='terraform fmt -check -recursive terraform && terraform -chdir=terraform init -backend=false -input=false -lockfile=readonly && sh ci/cloud-compose-snapshot-contract_test.sh && sh ci/cloud-compose-release-contract_test.sh && terraform -chdir=terraform validate'
container_id="$(
  docker create \
    --workdir /repo \
    --entrypoint sh \
