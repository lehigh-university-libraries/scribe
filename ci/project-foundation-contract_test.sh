...
for service in servicemanagement.googleapis.com serviceusage.googleapis.com; do
  grep -Fq "\"${service}\"" terraform/foundation/main.tf ||
    fail "foundation does not own ${service}"
done
control_plane_service="$(sed -n '/^resource "google_project_service" "control_plane" {/,/^}/p' terraform/foundation/main.tf)"
printf '%s\n' "$control_plane_service" | grep -Eq 'disable_on_destroy[[:space:]]*=[[:space:]]*false' ||
...
service_usage_bootstrap="$(printf '%s\n' "$foundation_job" | sed -n '/^[[:space:]]*apply)$/,/^[[:space:]]*;;/p')"
# shellcheck disable=SC2016 # Match the literal workflow variable reference.
printf '%s\n' "$service_usage_bootstrap" | grep -Fq 'gcloud services enable serviceusage.googleapis.com --project="$GCLOUD_PROJECT" --quiet' ||
  fail "protected foundation apply does not bootstrap the Service Usage API"
if printf '%s\n' "$foundation_job" | sed -n '/^[[:space:]]*plan)$/,/^[[:space:]]*;;/p' | grep -q 'gcloud services enable'; then
  fail "foundation plan can mutate the project API set"
fi
service_usage_line="$(printf '%s\n' "$foundation_job" | grep -nF 'gcloud services enable serviceusage.googleapis.com' | cut -d: -f1)"
terraform_init_line="$(printf '%s\n' "$foundation_job" | grep -nF 'terraform -chdir=terraform/foundation init' | cut -d: -f1)"
[ -n "$service_usage_line" ] && [ -n "$terraform_init_line" ] && [ "$service_usage_line" -lt "$terraform_init_line" ] ||
  fail "Service Usage must be enabled before Terraform initializes the foundation"
