...
if command -v actionlint >/dev/null 2>&1 && host_actionlint_is_pinned; then
  actionlint "${workflow_files[@]}"
elif command -v docker >/dev/null 2>&1; then
  if command -v actionlint >/dev/null 2>&1; then
    echo "Installed actionlint is not ${EXPECTED_ACTIONLINT_VERSION}; using the pinned container." >&2
  fi
  actionlint_image="rhysd/actionlint:1.7.12@sha256:b1934ee5f1c509618f2508e6eb47ee0d3520686341fec936f3b79331f9315667"
  actionlint_id="$(docker create -w /repo "${actionlint_image}" "${workflow_files[@]}")"
