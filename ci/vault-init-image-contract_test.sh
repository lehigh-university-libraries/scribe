...
fail() {
  echo "Vault init image contract failed: $*" >&2
  exit 1
}
...
for requirement in \
  ...
  'COPY scripts/vault-retry.sh /usr/local/lib/scribe/vault-retry.sh'; do
  rg -Fq "$requirement" Dockerfile || fail "Dockerfile is missing $requirement"
done

if rg -q 'apk (add|update)' scripts/vault-init.sh; then
  fail "vault-init installs mutable packages while handling deployment secrets"
fi
...
echo "Vault init image contracts passed."
