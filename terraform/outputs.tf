...
output "backend_readiness_job" {
  ...
  value       = try(google_cloud_run_v2_job.backend_readiness[0].name, "")
}

output "ocr_readiness_job" {
  description = "Cloud Run job that sends a synthetic image through private OCR endpoints."
  value       = try(google_cloud_run_v2_job.ocr_readiness[0].name, "")
}
...
output "vault_gcp_auth_key_verifier_role" {
  ...
  value       = local.vault_gcp_auth_key_verifier_role
}
...
output "foundation_workspace" {
  ...
  value       = local.foundation_state_prefix
}
...
output "vault_key_bucket" {
  ...
  value       = local.vault_is_owner_workspace ? module.vault[0].key_bucket : ""
}
...
output "internal_artifact_registry_repository" {
  ...
  value       = try(data.terraform_remote_state.shared_foundation.outputs.artifact_registry_repository_id, "")
}

output "mariadb_backup_disk" {
  description = "Independent production disk for bounded logical MariaDB dumps."
  value = local.is_prod_workspace ? {
    name            = google_compute_disk.mariadb_backups[0].name
    self_link       = google_compute_disk.mariadb_backups[0].self_link
    size_gb         = google_compute_disk.mariadb_backups[0].size
    retained_copies = local.mariadb_backup_retained_completed_copies
  } : null
}
...
output "backup_restore_verifier_role" {
  ...
  value       = local.is_prod_workspace ? google_project_iam_custom_role.backup_restore_verifier[0].name : ""
}
