...
variable "compose_network_cidr" {
  ...
  validation {
    condition = (
      ...
      length(regexall("/(2[4-9])$", var.compose_network_cidr)) == 1 &&
      !startswith(var.compose_network_cidr, "169.254.")
    )
    error_message = "compose_network_cidr must be a non-link-local IPv4 CIDR between /24 and /29 with room for Traefik at host 2."
  }
}
