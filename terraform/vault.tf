...
resource "google_service_account_iam_member" "vault_gcp_auth_app_service_account_key_viewer" {
  count = local.vault_is_owner_workspace ? 1 : 0

  service_account_id = module.scribe.appGsa.name
  role               = "roles/iam.serviceAccountKeyViewer"
  member             = "serviceAccount:${module.vault[0].gsa}"
}

resource "google_service_account_iam_member" "vault_gcp_auth_instance_service_account_key_viewer" {
  count = local.vault_is_owner_workspace ? 1 : 0
