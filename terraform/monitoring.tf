locals {
  readiness_job_names = toset(compact([
    try(google_cloud_run_v2_job.backend_readiness[0].name, ""),
    try(google_cloud_run_v2_job.ocr_readiness[0].name, ""),
  ]))
}
...
resource "google_monitoring_alert_policy" "readiness_job_failures" {
  for_each = local.is_prod_workspace ? local.readiness_job_names : toset([])
...
resource "google_monitoring_alert_policy" "persistent_disk_utilization" {
  ...
  documentation {
    content   = "A production filesystem exceeded 80% utilization, or disk-utilization telemetry disappeared. The independently sized MariaDB logical-backup disk must always retain capacity for its completed dump plus a full staging dump."
    mime_type = "text/markdown"
  }
