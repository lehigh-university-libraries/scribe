...
sync_database_secrets_from_vault() {
  ...
  source .env
  local vault_addr vault_role vault_kv_mount vault_secret_prefix vault_database_app_path vault_database_root_path host_uid host_gid
  ...
  vault_secret_prefix="${VAULT_SECRET_PREFIX:-$DEFAULT_VAULT_SECRET_PREFIX}"
