...
OAuth login state is stored in-process. The supported deployment shape is a
single backend instance for browser OAuth; running multiple backend instances
without sticky sessions or a shared state store can break callback validation.
...
workspace-scoped Vault KV v2 `secret/data/scribe/dev/database/app` and
`secret/data/scribe/dev/database/root` secrets by default before Docker Compose starts
