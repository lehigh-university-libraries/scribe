...
export function renderEditorLayout(app: HTMLElement): void {
  setHTML(app, html`
    <main class="flex h-dvh min-h-0 w-screen flex-col overflow-hidden bg-background text-foreground">
      <header class="flex flex-none flex-wrap items-center justify-between gap-2 border-b border-border bg-background/95 px-4 py-2">
      ...
      <section class="relative min-h-0 flex-1 overflow-hidden">
        <div id="editor-batch-banner" role="status" aria-live="polite" class="hidden pointer-events-none absolute inset-x-0 top-0 z-40 px-4 py-4">
        ...
        <div id="mirador-viewer" class="h-full min-h-0 w-full"></div>
      </section>
      <div id="leave-dialog" role="dialog" aria-modal="true" aria-labelledby="leave-dialog-title" class="hidden fixed inset-0 z-50 items-center justify-center bg-foreground/20">
        <div class="w-full max-w-md rounded-lg border border-border bg-card p-6 text-card-foreground shadow-2xl">
          <h2 id="leave-dialog-title" class="text-lg font-semibold">Leave editor?</h2>
          <p class="mt-2 text-sm text-muted-foreground">You have unsaved changes. Save before returning home?</p>
          <div class="mt-5 flex justify-end gap-2">
            <button type="button" id="leave-cancel" class="inline-flex items-center gap-2 rounded-md border bg-background px-3.5 py-2 text-sm font-medium text-foreground shadow-xs transition hover:bg-accent hover:text-accent-foreground disabled:pointer-events-none disabled:opacity-50">Cancel</button>
            <button type="button" id="leave-discard" class="inline-flex items-center gap-2 rounded-md border bg-background px-3.5 py-2 text-sm font-medium text-destructive shadow-xs transition hover:bg-destructive/10 disabled:pointer-events-none disabled:opacity-50">Discard</button>
            <button type="button" id="leave-save" class="inline-flex items-center gap-2 rounded-md bg-primary px-3.5 py-2 text-sm font-medium text-primary-foreground transition hover:bg-primary/90 disabled:pointer-events-none disabled:opacity-50">Save</button>
          </div>
        </div>
      </div>
    </main>
