...
import { getAuthMe, listAPIKeys, listProviderSecrets, logout } from "../api/auth";
...
async function setupShell(): Promise<HTMLElement> {
  ...
  vi.mocked(subscribeToEvents).mockReturnValue({ close: vi.fn() });
