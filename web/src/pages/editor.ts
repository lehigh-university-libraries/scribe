...
import { setHTML, uint64ToString } from "../lib/util";
...
export async function renderEditor(app: HTMLElement): Promise<void> {
  ...
  if (!runResp.imageUrl || runResp.imageUrl.trim() === "") {
    ...
    if (viewer) {
      setHTML(viewer, `<div class="flex h-full items-center justify-center text-sm text-muted-foreground">No image is available for this OCR run.</div>`);
    }
    return;
  }
