...
function assertSavedStructuralPage(annotationPage, expectedStructure) {
  const expectedWordTexts = [...deterministicLineText.split(" "), "epsilon"];
  const expectedLineText = expectedWordTexts.join(" ");
  ...
  if (
    !expectedLine?.id
    || annotationTextValue(expectedLine) !== expectedLineText
    || expectedWords.length !== expectedWordTexts.length
    || new Set(expectedWords.map((annotation) => annotation?.id)).size !== expectedWords.length
    || !savedLine
    || annotationTextValue(savedLine) !== expectedLineText
    ...
    || !wordsAreOwned
    || orderedWords.some((annotation, index) => annotationTextValue(annotation) !== expectedWordTexts[index])
  ) {
...
async function warmInitialBrowserIngress() {
  ...
  while (true) {
    ...
    } else if (status === 429) recordBrowserFault("rate");
...
async function findProductionReadinessOrphans(workspaceID, recoveryDeadline) {
  ...
  if (
    !Array.isArray(keyPayload?.apiKeys)
    || keyPayload.apiKeys.length > productionJanitorMaxAPIKeys
  ) {
    throw new Error("production janitor token inventory bound exceeded");
  }
  const seenKeyIDs = new Set();
  for (const key of keyPayload.apiKeys) {
    if (
...
async function currentEditorStructuralSnapshot() {
  const snapshot = await page.waitForFunction(() => {
    ...
    const line = annotations.find((annotation) => (
    ...
    return line && words.length > 0 ? { line, words } : undefined;
  });
...
try {
  ...
  page.on("response", (response) => {
    ...
    if (isStartUploadBatchResponse) {
      ...
      if (response.status() >= 400) recordBrowserFault("upload");
      return;
    }
