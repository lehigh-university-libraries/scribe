...
- hash-locked segmentor Python transitives and exact-digest high/critical Trivy
  scans of every built backend, frontend, OCR, and Terraform-built Vault image;
- Terraform formatting/init/validation and operational identity, action-pin,
  ...
  readiness-fixture, and backup-policy contracts;
...
- release-chain fixtures that reject dispatch or pushed-tag credential access,
  PR-head release checkouts, and legacy tag workflows, plus a fake Git/GitHub
  retry proving a later main HEAD is rejected, a partial publish reuses the
  exact unmoved numeric tag and draft, a completed release is a no-op, and an
  out-of-order older merge skips only after a later exact release is complete;
...
dependency and secret scan as the hosted workflow. Individual component commands remain
useful for iteration, but a manually checked box is not a substitute for a
passing required job. The orchestrator creates a unique Compose project, waits
...
for the production data, Compose-volume, and independent logical-backup disk;
read-only/no-egress inspection; completion-marker and schema checks; safe
label/provenance-gated cleanup; stale resources; and partial-failure cleanup.

`make ocr-build-tags` also runs the Kraken installer contract. It proves that a
