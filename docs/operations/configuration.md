...
`172.30.0.2` on a dedicated bridge and trusts only `172.30.0.2/32`. Traefik
does not trust local direct callers to provide forwarding headers. Terraform
...
trust boundary. If `SCRIBE_COMPOSE_SUBNET` or `SCRIBE_TRAEFIK_IP` is
overridden locally, update `SERVER_TRUSTED_PROXY_CIDRS` to the matching exact
Traefik `/32`; invalid CIDRs fail startup.
...
image. The Vault image similarly pins the official HashiCorp archive digest;
version bumps must update the version and checksum together.
