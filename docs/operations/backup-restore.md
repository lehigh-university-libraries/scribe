...
- A dedicated hyperdisk stores logical MariaDB dumps independently from the
  live data and Docker-volume disks. Its capacity is three times the live data
  disk: one retained complete dump, one full staging dump, and one full-dump
  margin. The writer stages atomically, verifies gzip content and a completion
  marker, then retains the newest complete copy.
- Daily and weekly immutable snapshot policies cover the live data disk,
  Compose-volume disk, and logical-backup disk. Logical history therefore
  survives loss or corruption of the VM and live disks.
...
source-matched snapshots for all three production disks, creates three distinct
...
2. Run the protected backup workflow. Its required three-disk probe proves that
   the live database volume, persistent Compose volume, and completed logical
   dump can be restored together without production credentials or egress.
