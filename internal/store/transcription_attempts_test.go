...
func TestTranscriptionAttemptCannotOverwriteNewerHumanCorrection(t *testing.T) {
	databasePool := annotationTestDB(t)
	ctx := context.Background()
	suffix := uuid.NewString()
	canvasURI := "https://source.example/canvas/input-revision-fence-" + suffix
	processingContext := createAnnotationTestContext(t, databasePool, suffix+"-input-revision-fence")
	workspaceID, imageID := createAnnotationTestResource(t, databasePool, suffix+"-input-revision-fence", canvasURI)
	annotationStore := store.NewAnnotationStore(databasePool)
	jobStore := store.NewTranscriptionJobStore(databasePool)

	inputPage, err := annotationStore.SavePage(
		ctx,
		canonicalTestPage(t, workspaceID, imageID, canvasURI, "worker input"),
		0,
	)
	if err != nil {
		t.Fatalf("SavePage: %v", err)
	...
	if fence.InputRevision != inputPage.Revision {
		t.Fatalf("fence input revision = %d, want %d", fence.InputRevision, inputPage.Revision)
	}

	humanCorrection := inputPage
	humanCorrection.Payload = replacePageText(t, inputPage.Payload, "newer human correction")
	humanSaved, err := annotationStore.SavePage(ctx, humanCorrection, inputPage.Revision)
	if err != nil {
		t.Fatalf("save human correction: %v", err)
	}

	workerOutput := inputPage
	workerOutput.Payload = replacePageText(t, inputPage.Payload, "stale worker output")
	if _, err := annotationStore.SavePageAndCompleteTranscriptionJob(
		ctx,
		workerOutput,
		humanSaved.Revision,
		store.AnnotationJobCompletion{TranscriptionAttemptFence: fence},
	); !errors.Is(err, store.ErrAnnotationJobFence) {
		t.Fatalf("completion with advanced expected revision error = %v, want ErrAnnotationJobFence", err)
	}
	if _, err := annotationStore.SavePageAndCompleteTranscriptionJob(
		ctx,
		workerOutput,
		...
		t.Fatalf("completion at stale input revision error = %v, want ErrAnnotationRevisionConflict", err)
	}

	reloaded, err := annotationStore.LoadPage(ctx, workspaceID, imageID)
	if err != nil {
		t.Fatalf("LoadPage after stale completions: %v", err)
	}
	if reloaded.Revision != humanSaved.Revision {
		t.Fatalf("canonical revision = %d, want human revision %d", reloaded.Revision, humanSaved.Revision)
	}
	assertPageText(t, reloaded.Payload, "newer human correction")

	job, err := jobStore.Get(ctx, jobID)
	if err != nil {
		t.Fatalf("Get transcription job: %v", err)
	}
	if job.Status == store.TranscriptionJobStatusCompleted {
		t.Fatalf("stale attempt completed job: %+v", job)
	}
	if len(job.Attempts) != 1 || job.Attempts[0].InputRevision != inputPage.Revision || job.Attempts[0].ResultRevision != nil {
		t.Fatalf("stale attempt audit changed = %+v", job.Attempts)
	}
}
