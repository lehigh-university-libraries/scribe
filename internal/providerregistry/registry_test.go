...
func TestGemini3DoesNotAdvertiseOrAcceptDeprecatedTemperature(t *testing.T) {
	...
	for _, provider := range registry.Catalog().TranscriptionProviders {
		if provider.ID == "gemini" && provider.SupportsTemperature {
			t.Fatal("mixed Gemini 3.x catalog advertised provider-wide temperature support")
		}
	}

	legacyConfig := config.Config{}
	legacyConfig.LLM.Gemini.Model = "gemini-2.5-flash"
	legacyConfig.LLM.Gemini.Models = []string{"gemini-2.5-flash"}
	for _, provider := range New(legacyConfig).Catalog().TranscriptionProviders {
		if provider.ID == "gemini" && !provider.SupportsTemperature {
			t.Fatal("legacy-only Gemini catalog omitted supported temperature capability")
		}
	}
}
