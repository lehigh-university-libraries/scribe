...
// Option configures private runtime dependencies without changing registered
// provider endpoints, audiences, models, credentials, or limits.
type Option func(*registryOptions)

// WithVendorHTTPClient supplies the transport used by fixed-vendor adapters.
// The HTR clients clone it and continue to enforce their own redirect and
// timeout policy. Registered provider URLs remain authoritative.
func WithVendorHTTPClient(client *http.Client) Option {
	return func(options *registryOptions) {
		options.vendorHTTPClient = client
	}
}
...
func New(cfg config.Config, configure ...Option) Registry {
	...
	for _, descriptor := range providerDescriptors {
		...
		if descriptor.Endpoint.Mode == EndpointVendor {
			descriptor.vendorHTTPClient = options.vendorHTTPClient
		}
...
func (r Registry) Catalog() Catalog {
	...
	for _, id := range r.providerOrder {
		...
		for _, model := range descriptor.Models {
			if !providerModelSupportsTemperature(descriptor, model.ID) {
...
func (r Registry) ResolveSegmentor(selection string) (Segmentor, string, string, error) {
	...
	registeredSelection, ok := resolveModel(r.segmentationModels, selection)
	if !ok {
		return Segmentor{}, "", "", fmt.Errorf("unsupported segmentation model %q", selection)
	}
	selection = registeredSelection.ID
