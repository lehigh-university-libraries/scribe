...
func TestWordStructuralLifecyclePersistsAcrossCanonicalPublicAndExports(t *testing.T) {
	...
	for _, resource := range resources {
		if resource.ID == published.Msg.GetPublicUrl() {
			publicPage = string(resource.Payload)
			break
		}
	}
	if publicPage == "" {
		t.Fatalf("published Triplet graph omitted canonical page %q", published.Msg.GetPublicUrl())
	}
