...
func TestOversizedManifestIsRejectedBeforeTenantContentWrite(t *testing.T) {
	...
	t.Cleanup(func() {
		_, _ = database.ExecContext(ctx, `DELETE FROM items WHERE source_url = ?`, manifestSource.URL+"/manifest")
		_, _ = database.ExecContext(ctx, `DELETE FROM external_requests WHERE workspace_id = ? AND source = 'item-create' AND idempotency_key = ?`, store.AnonymousWorkspaceID, storedKey)
	})
...
func TestImportManifestCompletedReplayDoesNotRefetchSource(t *testing.T) {
	...
	storedKey := fmt.Sprintf("%x", digest[:])
	t.Cleanup(func() {
		_, _ = database.ExecContext(context.Background(), `DELETE FROM items WHERE source_url = ?`, manifestURL)
		_, _ = database.ExecContext(context.Background(), `
DELETE FROM external_requests
WHERE workspace_id = ? AND source = 'item-create' AND idempotency_key = ?`, store.AnonymousWorkspaceID, storedKey)
	})
