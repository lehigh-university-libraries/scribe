...
func TestListItemsHandlerContinuesWithoutDuplicates(t *testing.T) {
	...
	for index, itemID := range itemIDs {
		if _, err := itemStore.Create(context.Background(), dbstore.CreateItemParams{
			ID: itemID, UserID: userID, WorkspaceID: workspaceID, Name: itemNames[index], SourceType: "upload",
		}); err != nil {
			t.Fatalf("create %s: %v", itemID, err)
		}
		if _, err := database.Exec(`UPDATE items SET created_at = ?, updated_at = ? WHERE workspace_id = ? AND id = ?`, createdAt, createdAt, workspaceID, itemID); err != nil {
			t.Fatalf("set deterministic timestamps for %s: %v", itemID, err)
		}
	}
	if _, err := itemStore.AddImage(context.Background(), dbstore.CreateItemImageParams{
		ItemID:    itemIDs[0],
		Sequence:  0,
		ImageURL:  "https://images.example/first.jpg",
		CanvasURI: "https://source.example/canvas/item-page-first",
		Label:     "First page",
	}); err != nil {
		t.Fatalf("create preview image: %v", err)
	}
