...
type GetOCRRunByItemImageIDManualRow struct {
...
func (q *Queries) GetOCRRunByItemImageIDManual(ctx context.Context, itemImageID sql.NullInt64) (GetOCRRunByItemImageIDManualRow, error) {
	row := q.db.QueryRowContext(ctx, getOCRRunByItemImageIDManual, itemImageID)
	var i GetOCRRunByItemImageIDManualRow
	err := row.Scan(
		&i.SessionID,
		&i.ItemImageID,
		&i.ContextID,
...
type GetOCRRunManualRow struct {
	SessionID           string         `json:"session_id"`
	ItemImageID         sql.NullInt64  `json:"item_image_id"`
	ContextID           sql.NullInt64  `json:"context_id"`
	ImageUrl            string         `json:"image_url"`
...
func (q *Queries) GetOCRRunManual(ctx context.Context, sessionID string) (GetOCRRunManualRow, error) {
	row := q.db.QueryRowContext(ctx, getOCRRunManual, sessionID)
	var i GetOCRRunManualRow
	err := row.Scan(
		&i.SessionID,
		&i.ItemImageID,
		&i.ContextID,
...
const saveOCREditsManual = `-- name: SaveOCREditsManual :exec
UPDATE ocr_runs
SET
  corrected_hocr = ?,
  corrected_text = ?,
...
func (q *Queries) SaveOCREditsManual(ctx context.Context, arg SaveOCREditsManualParams) error {
	_, err := q.db.ExecContext(ctx, saveOCREditsManual,
		arg.CorrectedHocr,
		arg.CorrectedText,
		arg.EditCount,
