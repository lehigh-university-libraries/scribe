...
func (q *Queries) ClaimNextLeasedTranscriptionJob(ctx context.Context, leaseUntil time.Time, lockedBy string) (TranscriptionJob, error) {
	...
	if err := q.MarkTranscriptionJobLeasedManual(ctx, MarkTranscriptionJobLeasedManualParams{
...
func (q *Queries) InsertEventOutbox(ctx context.Context, eventID, eventType string, subject sql.NullString, bodyJSON string) error {
