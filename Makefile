.PHONY: help
.PHONY: build build-frontend frontend-image-smoke vault-init-image-smoke fmt fmt-check lint toolchain-check test test-backend test-frontend test-browser e2e-smoke backup-restore-smoke verify-cloud-backups-test cloud-snapshot-restore-drill-test mariadb-backup-retention-test preview-deployment-test readiness-fixture-test deployment-status-test reset-dev-db-test ocr-build-tags segmentor-lock segmentor-lock-check export-schema-check proto proto-lint sqlc generate generate-check security dependency-scan ops-security-contracts terraform-check docs docs-serve install-tools install-shell-tools install-codegen-tools install-security-tools install-doc-tools doctor ci up up-db reset-dev-db down logs sequelace ocr-matrix ocr-images tf-dev tf-dev-vault tf-dev-ocr tf-prod tf-prod-ocr tf-preview vault-secrets
...
ocr-matrix: ## Print the OCR build matrix derived from config/ocr.yaml. Usage: GCLOUD_PROJECT=... WORKSPACE_SLUG=prod IMAGE_TAG=<commit> make ocr-matrix
	@./ci/ocr-matrix.sh

ocr-images: ## Resolve/build GAR OCR images from config/ocr.yaml. Usage: GCLOUD_PROJECT=... WORKSPACE_SLUG=prod IMAGE_TAG=<commit> [AUTO_BUILD_MISSING=true] make ocr-images
...
docs: ## Build the Zensical documentation site
	@./ci/docs.sh build
...
tf-prod: ## Run local Terraform for production. Usage: make tf-prod BRANCH=<40-character-sha> ACTION=plan|apply|destroy
	@set -eu; \
	action="${ACTION}"; \
	if [ -z "$$action" ]; then action="plan"; fi; \
	branch_arg=""; \
...
tf-preview: ## Run local Terraform for a preview env. Usage: make tf-preview PR=23 BRANCH=<40-character-base-sha> ACTION=plan|apply|destroy
	@set -eu; \
	action="${ACTION}"; \
	if [ -z "$$action" ]; then action="plan"; fi; \
	pr="${PR}"; \
